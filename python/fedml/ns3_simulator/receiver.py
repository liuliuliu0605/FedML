...
class Receiver(ns.network.Application):
    ...
    def new_connection_created(socket: ns.Socket, address: ns.Address) -> None:
        ...
        # print("new_connection_created:", Receiver.object_dict[obj_key]._id, ns.core.Simulator.Now().GetSeconds())
    ...
    def rcv_packet(self, socket):
        ...
        while self.offline_or_online == 1:
            ...
            if self.verbose:
                print("+ At time %.6f packet sink %s (receiver %s) received %d (%d) bytes from %s port %d" %
    ...
    def complete_one_phase(self, socket, address):
        ...
        if self.communicator is not None:
            self.communicator.after_phase()

        if self.verbose:
            print("+ In %d-th phase packet sink %s (receiver %s) received %d total bytes" %
                  (self.current_phase - 1, self.communicator.get_id(), self._id, self.currentRxSize))

        # finish all phases
        if self.current_phase >= self.phases:
            if self.verbose:
                print("[Reception Finished] At time %.6f packet sink %s (receiver %s) received %d total bytes from %s port %d" %
    ...
    def go_offline(self):
        ...
        if self.verbose:
            print("# At time %.6f packet sink %s (receiver %s) is offline" %
    ...
    def Setup(self, local_socket_addr, phaseTxSize, phases=1, _id='-1', offline_or_online=1, protocol='tcp', verbose=False):
        ...
        self._create_socket()
        self.phaseRxSize = int(phaseTxSize)
...
if __name__ == "__main__":
    ...
    communicator = Communicator(nodes.Get(0), _id='1')
    ...
    if app_receiver.is_finished and app_sender.is_finished:
        print("done!")
