...
class Communicator:
    ...
    def add_app_sender(self, dst_communicator, phase_rx_size, phases, port, packet_size, start_time=0, stop_time=None):
        ...
        app_sender.Setup(sink_address, phase_rx_size, phases=phases, packetSize=packet_size, _id=communicator_id,
                         offline_or_online=1, protocol=self.protocol, verbose=self.verbose)
    ...
    def update_phase(self):
        ...
        if is_update:
            if self.verbose:
                print("Node %d entered %d-th phase" % (self._id, self.next_phase))
    ...
    def generate_message(self, message=None):
        ...
        message = self.next_phase if message is None else message
        for sender in self.app_sender_dict.values():
