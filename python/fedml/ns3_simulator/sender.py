...
class Sender(ns.network.Application):
    ...
    def check_before_sending(self):
        ...
        if len(self.message_queue) == 0:
            ...
            ready = False
            warning_message = "Warning: node %d (sender %d) was offline!" % (self.communicator.get_id(), self._id)
    ...
    def go_offline(self):
        self.current_time = ns.core.Simulator.Now().GetSeconds()
        if self.verbose:
            print("# At time %.6f packet source %d (sender %d) is offline" %
    ...
    def add_message(self, msg=None):
        ...
        if len(self.message_queue) == 1 and self.is_idle:
            ...
            self.write_until_buffer_full()
        if self.verbose:
            print("Node %d (sender %d) receives new message" % (self.communicator.get_id(), self._id))
    ...
    def fast_forward(self):
        ...
        if self.verbose:
            print("Fake: node %d sending data to node %d" % (self.communicator.get_id(), self._id))
...
if __name__ == "__main__":
    ...
    app.Setup(sinkAddress, int(1e3), phases=1, packetSize=1448, _id=-1, offline_or_online=1,
              protocol=protocol, verbose=verbose)
    nodes.Get(0).AddApplication(app)

    app.SetStartTime(ns.core.Seconds(0))
    app.SetStopTime(ns.core.Seconds(200))
